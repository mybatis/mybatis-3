  ...
  protected MixedSqlNode parseDynamicTags(XNode node) {
    ...
    for (int i = 0; i < children.getLength(); i++) {
      ...
      if (child.getNode().getNodeType() == Node.CDATA_SECTION_NODE || child.getNode().getNodeType() == Node.TEXT_NODE) {
        ...
        if(configuration.containsSqlNode(data)) {
        	SqlNode sqlNode = configuration.getSqlNode(data);
			...
			if (textSqlNode.isDynamic()) {
				...
				isDynamic = true;
				configuration.addSqlNode(data, textSqlNode);
			} else {
				StaticTextSqlNode staticTextSqlNode = new StaticTextSqlNode(data);
				contents.add(staticTextSqlNode);
				configuration.addSqlNode(data, staticTextSqlNode);
			}
        }
      } else if (child.getNode().getNodeType() == Node.ELEMENT_NODE) { // issue #628
    	  String str = child.toString();
    	  if(configuration.containsSqlNode(str)) {
    		 contents.add(configuration.getSqlNode(str));
    		 isDynamic = true;
    	  }else {
    		  ...
    		  configuration.addSqlNode(str,sqlNode);
    		  contents.add(sqlNode);
    		  isDynamic = true;
    	  }
      	}
