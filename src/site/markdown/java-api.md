...
Or with an environment or properties:

```java
SqlSessionManager manager = SqlSessionManager.newInstance(inputStream, environment);

// ... or ...

SqlSessionManager manager = SqlSessionManager.newInstance(inputStream, properties);
```
...
SqlSessionFactory has eight methods that are used to create SqlSession instances. In general, the decisions you'll be making when selecting one of these methods are:
...
Finally, there are five advanced versions of the `select` methods that allow you to restrict the range of rows to return, or provide custom result handling logic, usually for very large data sets.
...
The `ResultContext` parameter gives you access to the result object itself, a count of the number of result objects created, and a `stop()` method that you can use to stop MyBatis from loading any more results.
...
public interface AuthorMapper {
  ...
  // (Map<Integer,Author>) selectMap("selectAuthorsAsMap", "id")
  @MapKey("id")
  Map<Integer, Author> selectAuthorsAsMap();
...
Since the very beginning, MyBatis has been an XML driven framework. The configuration is XML based, and the Mapped Statements have been defined in XML. With MyBatis 3, there are new options available. MyBatis 3 builds on top of a comprehensive and powerful Java based Configuration API. This Configuration API is the foundation for the XML based MyBatis configuration, as well as annotation-based configuration. Annotations offer a method to implement mapped statements without introducing the overhead of XML.

<span class="label important">NOTE</span> Java annotations are limited in their expressiveness and flexibility. Because of this, the configuration of complex result maps is not as expressive as XML. By using a combination of `@NamedResultMap` annotations, and the ability of `@Arg`, `@One` and `@Many` to refer to other result maps by name, we believe that most of what can be achieved in XML is also possible with annotations.
...
| `@NamedResultMap`                                                                                                 | `Type`      | `<resultMap>`                                                                         | This annotation is used to create a result map that is not tied directly to a select statement and can be reused by multiple statements. This is analogous to creating a `<resultMap>` in XML. The annotation should be attached to a type (typically a mapper interface). The annotation can accept an array of `@Arg`, and/or an array of `@Result`, and/or a `@TypeDiscriminator`. Any number of `@NamedResultMap` annotations can be attached to a type.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `@ResultMap`                                                                                                      | `Method`    | N/A                                                                                   | This annotation is used to provide the id of a `<resultMap>` element in an XML mapper, or the id of `@NamedResultMap` or `@Results`, to a `@Select` or `@SelectProvider` annotation. This allows annotated selects to reuse resultmaps that are defined in XML. This annotation will override any `@Results` or `@ConstructorArgs` annotation if both are specified on an annotated select.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `@ResultOrdered`                                                                                                  | `Method`    | "resultOrdered" attribute of select XML                                               | This annotation is used to denote that a result set is known to be ordered. This is required when building object that have collection in their constructors                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
